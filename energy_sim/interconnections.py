...
- :class:`Interconnection` — static definition of a commercial border:
  ...
  :class:`~energy_sim.price_areas.PriceArea`, and a
  :class:`~energy_sim.reliability.ReliabilityModel`.
...
:func:`~energy_sim.dispatch.dispatch_year` (``srmc()``,
...
:mod:`~energy_sim.dispatch`) using the realization's export NTC and
...
class Interconnection:
    ...
    Attributes:
        ...
        reliability_model (ReliabilityModel): Stochastic availability
            model. Use :class:`~energy_sim.reliability.PerfectReliability`
    ...
    def realize(self, foreign_price: np.ndarray,
        ...
        the :class:`~energy_sim.price_areas.PriceAreaCoupling`) with a
        ...
        Args:
            ...
            carbon_intensity_g_per_kwh: Average emission intensity of the
                ...
                :class:`~energy_sim.price_areas.PriceArea`.
...
class InterconnectionRealization:
    ...
    def as_virtual_import_generator(self) -> "VirtualImportGenerator":
        ...
        :func:`~energy_sim.dispatch.dispatch_year`. This is the mechanism
...
class VirtualImportGenerator:
    ...
    :func:`~energy_sim.dispatch.dispatch_year` reads from a
    :class:`~energy_sim.generators.Generator`, so that imports can be
...
def build_interconnections_from_config(
    ...
    :func:`~energy_sim.reliability.build_reliability_model`. When
    ``enable_faults`` is ``False``, every interconnection is forced to
    :class:`~energy_sim.reliability.PerfectReliability` regardless of its
    config entry — this implements the
    :data:`~energy_sim.config.ENABLE_NTC_FAULTS` master switch.
...
def build_coupling_for_interconnections(
    ...
    :func:`~energy_sim.price_areas.build_price_areas_from_config` with the
    appropriate subset. Avoids simulating unused areas.

    Args:
        interconnections: Iterable of :class:`Interconnection` objects.
        price_areas_cfg: See :data:`~energy_sim.config.PRICE_AREAS`.
        correlations_cfg: See :data:`~energy_sim.config.PRICE_AREA_CORRELATIONS`.
