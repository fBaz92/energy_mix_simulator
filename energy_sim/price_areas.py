...
The master switch :data:`~energy_sim.config.PRICE_AREAS_CORRELATED` allows
...
class _PricePathGenerator(Protocol):
    ...
    Matches :class:`~energy_sim.generators.FuelPriceModel` and
    :class:`~energy_sim.generators.ConstantFuelPrice`. Only the two methods
...
class PriceArea:
    ...
    :class:`~energy_sim.interconnections.Interconnection` instances can
    ...
    Attributes:
        ...
        price_model: Object implementing the price-path protocol
            (typically :class:`~energy_sim.generators.FuelPriceModel`).
...
def build_price_areas_from_config(
    ...
    This is the glue that consumes :data:`~energy_sim.config.PRICE_AREAS`
    and :data:`~energy_sim.config.PRICE_AREA_CORRELATIONS` and returns a
